...
class BaseLikelihood(ABC):
    ...
    def logpost(self, x):
        """
        Return the total log-posterior value at ``x``. The result is the sum of the backend's native log-probability, returned by ``backend_logpost``, and the additional bounds-based log-prior, returned by ``logprior``. Consequently, the result is negative infinity when ``x`` lies outside the current effective bounds.
        """
        lp = self.logprior(x)
