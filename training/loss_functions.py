...
def delta_chi2_from_k(k, n):
    k = tf.cast(k, tf.float32)
    n = tf.cast(n, tf.float32)

    mu = 1.0 - 2.0 / (9.0 * n)
    sigma = tf.sqrt(2.0 / (9.0 * n))
    return n * tf.pow(mu + k * sigma, 3.0)

def create_msre_loss(y_global_max, k, n, y_std):
    delta_chi2_k = delta_chi2_from_k(k, n)
...
def get_loss_function(loss_name, y_global_max=None, k=None, n=None, y_std=None):
    if loss_name == 'msre':
        return create_msre_loss(y_global_max, k, n, y_std)
